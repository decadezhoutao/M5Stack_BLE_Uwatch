...
#pragma mark - 上传数据到 WordPress
- (void)uploadDataToWordPress:(NSDictionary *)data {
    // 1. 设置 WordPress API 的 URL
    NSString *urlBase = @"https://24te533.edu2web.com/wp-json/wp/v2/posts";
    NSURL *url = [NSURL URLWithString:urlBase];

    // 2. 设置认证信息 (用户名和密码)
    NSString *user = @"24TE533 UPOD";
    NSString *password = @"sLmf 4ksQ cQxH Lmy2 CmUR zRw7";
    ...
    // 3. 根据数据内容判断类型并设置标签
    NSString *tagID = @"1"; // 默认设置为 Uncategorized 的 ID
    NSString *content = [NSString stringWithFormat:@"设备上传数据：%@", data];

    // 优先判断血氧数据
    if ([data[@"ST_GetRecordValueDataKey"] isKindOfClass:[NSString class]] && [data[@"ST_GetRecordIntervalKey"] integerValue] == 1) {
        NSLog(@"血氧数据: %@", data[@"ST_GetRecordValueDataKey"]);
        tagID = @"18"; // 血氧标签
    }
    // 判断步数和睡眠数据（统一为 StepAndSleep，标签ID为16）
    else if ([data[@"ST_GetRecordValueDataKey"] isKindOfClass:[NSArray class]] && [data[@"ST_GetRecordValueDataKey"] count] > 0) {
        NSDictionary *record = data[@"ST_GetRecordValueDataKey"][0]; // 只取第一个条目
        NSInteger type = [record[@"ST_GetRecordTypeStepOrSleepKey"] integerValue];
        if (type == 0) {
            // 这是步数和睡眠数据
            NSLog(@"步数: %@，睡眠: %@", record[@"ST_GetRecordValueStepKey"], record[@"ST_GetRecordValueSleepKey"]);
            tagID = @"16"; // StepAndSleep 标签
        }
    }
    // 区分心率和血压数据
    else if ([data[@"ST_GetRecordValueDataKey"] isKindOfClass:[NSString class]]) {
        NSString *valueData = data[@"ST_GetRecordValueDataKey"];
        NSArray *dataArray = [valueData componentsSeparatedByString:@" "]; // 按空格分割数据

        NSLog(@"数据数组长度: %lu", (unsigned long)[dataArray count]);

        // 判断数据量，多的为血压，少的为心率
        if ([dataArray count] > 2000) { // 如果总数量更多，判断为血压数据
            NSLog(@"血压数据: %@", valueData);
            tagID = @"19"; // 血压标签
        } else {
            NSLog(@"心率数据: %@", valueData);
            tagID = @"17"; // 心率标签
        }
    }
    ...
    NSDictionary *post = @{
        @"title": @"设备数据自动上传",
        @"status": @"publish",  // 可以使用 "draft" 或 "publish"
        @"content": content,
        @"tags": @[tagID]  // 设置标签的 ID
    };
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request
        ...
        if (error) {
            NSLog(@"上传数据到 WordPress 出错: %@", error);
        } else {
            NSHTTPURLResponse *httpResponse = (NSHTTPURLResponse *)response;
            if (httpResponse.statusCode == 201) {
                NSLog(@"数据成功上传到 WordPress");
            } else {
                NSLog(@"上传失败，状态码: %ld", (long)httpResponse.statusCode);
                // 输出错误信息
                if (data) {
...
// Helper method to get current date as ISO string
- (NSString *)getCurrentISODateString {
    ...
    if (error) {
        ...
        [STBlueToothData.sharedInstance notifyRunmefit:peripheral WriteCharacter:self.writeCharacter Characteristic:characteristic Error:error Complete:^(NSError * _Nonnull error, REV_TYPE revType, ERROR_TYPE errorType, id _Nonnull responseObject) {
            if (error) {
                ...
                // 调用上传至 WordPress 的方法
                [self uploadDataToWordPress:responseObject];
            }
        }];
    }
}
